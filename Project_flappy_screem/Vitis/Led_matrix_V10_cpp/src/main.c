...
static void prvLedMatrixTask( void *pvParameters )
{
	...
	for( ;; )
	{
    	Neopixel.setFrame(&frame1[0][0]);

    	usleep_A9(500000);
//    	vTaskDelay( x1second );

    	Neopixel.setFrame(&frame2[0][0]);

    	usleep_A9(500000);
//    	vTaskDelay( x1second );

    	Neopixel.setFrame(&frame3[0][0]);

    	usleep_A9(500000);

//    	BaseType_t gotSomething = xQueueReceive( 	xQueue,				/* The queue being read. */
//													&RecInt,	/* Data is read into this address. */
//													( TickType_t ) 1 );	/* Wait without a timeout for data. */
